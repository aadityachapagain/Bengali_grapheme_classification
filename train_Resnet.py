...
def train_model(model, dataloader,criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        epoch_loss = running_loss / (len(dataloader) * args.batch_size)
        ...
        if epoch_acc > best_acc:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            if args.save == 'yes':
                torch.save(model.state_dict(), f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Resnet')
...
if os.path.isfile(f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Resnet'):
    model_ft.load_state_dict(torch.load(f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Resnet'))
...
if args.overfit != 'yes':
    torch.save(model_ft.state_dict(), f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Resnet')