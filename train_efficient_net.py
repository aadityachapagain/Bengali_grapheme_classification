...
def train_model(model, dataloader,criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_acc > best_acc:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            if args.save == 'yes':
                torch.save(model.state_dict(), f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Efficient_Net')
...
print(f'Num of classess in {args.classifier} is {len(train_data_loader.get_classes())}')
dataloader = torch.utils.data.DataLoader(train_data_loader, batch_size=batch_size, shuffle=True, num_workers= 4)

model_ft = EfficientNet.from_pretrained('efficientnet-b7', num_classes=len(train_data_loader.get_classes()))


if os.path.isfile(f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Efficient_Net'):
    model_ft.load_state_dict(torch.load(f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Efficient_Net'))
...
torch.save(model_ft.state_dict(), f'../gdrive/My Drive/kaggle_bengali/{args.classifier}__model__Efficient_Net')